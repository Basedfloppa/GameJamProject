...
func _physics_process(delta):
	if velocity.y <= LIMIT_SPEED_Y:
		if !isDashing: velocity.y += GRAVITY * delta
	...
	if Input.is_action_just_pressed("jump"):
		if canJump:
			...
			if rayCast.is_colliding(): wallJump(delta)
...
func horizontalMovement(delta):
	if Input.is_action_pressed("ui_right"):
		...
		if canJump:
			animation.play(str(spriteColor, "Idle"))
...
func _on_trailTimer_timeout() -> void:
	if trail:
		...
		trail_sprite.scale = Vector2($Rotatable.scale.x * 2.4, $Rotatable.scale.y * 2.4)
		trail_sprite.set_script(load("res://assets/scripts/trail_fade.gd"))
		trail_sprite.position = position
		trail_sprite.modulate = Color.RED
